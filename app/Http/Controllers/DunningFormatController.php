...
namespace App\Http\Controllers;
    ...
    use File;
    use App\Page;
class DunningFormatController extends Controller
{
    // *Note
    ...
    {
        return view('dunning_query_format.dunning_format');
    }
                    ...
                    if($bill_cycle == "specific_bill")
                    {
                        ...
                        if(!empty($data_date)){
                            ...
                            if(!empty($insert_date))
                            {
                                ...
                                $stid = oci_parse($conn, "
                                select distinct
                                b4.cust_code,',', b2.acct_code,',', b1.msisdn,',,', 'remark'
                                from ccare.inf_subscriber_all b1,
                                   ccare.inf_acct           b2,
                                   ccare.inf_acct_relation  b3,
                                   ccare.inf_customer_all b4
                                where b1.sub_id = b3.sub_id
                                   and b2.acct_id = b3.acct_id
                                   and b4.cust_id = b2.cust_id
                                   and b1.exp_date > sysdate
                                   and b2.exp_date > sysdate
                                   and b3.exp_date > sysdate
                                   and b4.exp_date > sysdate
                                   and b1.msisdn in ('10202177') 
                                   ");
                                oci_execute($stid);
                                $total_data = array();
                                $final_data = [];
                                while ($row = oci_fetch_assoc($stid)) {
                                    ...
                                    $total_data['msisdn'] = $row['MSISDN'];
                                    $total_data['remark'] = $row['remark'];
