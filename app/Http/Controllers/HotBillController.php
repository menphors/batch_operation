...
class HotBillController extends Controller
{
    //
    var $cust_info=null;
    ...
    public function index(){
        ...
        return view('Hot_Bill.index',$customer_info);
    }
  

    public function HotBill_Operatrion(Request $request)
    {
    
        $cust_info = $request->file('number');
        $cust_info = Excel::toArray( new MsisdnsImport, $cust_info );
        // dd($cust_info[0][0][1]);
      $colrelationID=$cust_info[0][0][1];
        $timeStarts=Carbon::now();
        //==============================(AccountCode Line 500)=================================
        $status=null;
        $timeStart=null;
        $timeEnd=null;
        $error=$this->WrtieLogs_Group("HotBill",$colrelationID);
        $timeStart=Carbon::now();
        $cust_info1=$this->getAccount_Code_by_Number($cust_info[0][0][0]);
        ...
        $this->WrtieLogs("GetAcct_Code",$status,$timeStart,$timeEnd,$colrelationID,"NULL");
     
        $randDeomFileName= $colrelationID;
        $fileNameServer;
        ...
        for($i=1;$i<count($cust_info1) ; $i++){
            ...
            $this->WrtieLogs("Get Task_ID",$status,$timeStart,$timeEnd,$colrelationID,"NULL");
...
$fileName_PDF=substr( $result2_PDF['XML'],78,strlen( $result2_PDF['XML'])-78);      
        ...
        $this->WrtieLogs("Download XML by Task_ID",$status,$timeStart,$timeEnd,$colrelationID,"NULL");
           ...
           //===================================(GetExchangeRate from local DB: Line 549)============================
            ...
            $this->WrtieLogs("Get ExchangeRate",$status,$timeStart,$timeEnd,$colrelationID,"NULL");
            ...
            $this->WrtieLogs("Get InvoiceNo",$status,$timeStart,$timeEnd,$colrelationID,"NULL");
            ...
            $this->WrtieLogs("Get VAT",$status,$timeStart,$timeEnd,$colrelationID,"NULL");
            ...
            $this->WrtieLogs("Get Company Name",$status,$timeStart,$timeEnd,$colrelationID,"NULL");
            ...
            $this->WrtieLogs("Modify XML",$status,$timeStart,$timeEnd,$colrelationID,"NULL");
            ...
            $this->WrtieLogs("Generate PDF",$status,$timeStart,$timeEnd,$colrelationID,"NULL");

            //========================================(End Of HotBill Process)=======================
            $this->WrtieLogs("Generate HotBill",$status,$timeStarts,$timeEnd,$colrelationID,$location_PDF);

            $this->downloadingPDF($result_PDF['link'], $result1_PDF['link']);
            $fileName = $result_PDF['link'];
            return view ('Hot_Bill.Copy')->with(['fileName'=>$fileName]);
    ...
    public function replacePDF(){
        ...
        t.output_dir || t.output_filename,
        t.*
   from general.bb_billfile_desc_202012@bmpdb t --***
  where t.flow_type = 5
...
and t4.acct_code='".$ACCT."'");
    ...
    public function downloadingPDF($fileName, $file_dir){
        $file_dir = substr($file_dir, 11);
        // $new= '/onip/invbill_mount/bill_nfs/fmt'.$file_dir.$fileName;
        $testcontent = Storage::disk('sftp')->get('/onip/invbill_mount/bill_nfs/fmt'.$file_dir.$fileName);
        Storage::disk('local')->put('public/'.$fileName,$testcontent);
  ...
  and b.msisdn in ('".$phone_Number."')");
    ...
    public function generatePDF_By_XML($fileName,$localtionFileName,$randDeomFileName){
     ...
     public function getInvoicesNumber(){
        ...
        // oci_execute($stids);
       ///insert into invoice number code
...
// ======================== Modify XML ================================
        ...
        public function GetBackResultToClinet(Request $request){
            try{
                $Uploading= DB::table('tbproccesreport')
                ->select('Status')
                ->where('ProccesName','=','GetAcct_Code')
                ->where('colrelationID','=',$colrelationID)
                ->paginate(10);
                // dd($PDF[0]);
                if($Uploading[0]->Status=='Success'){
                try{
                $PDF= DB::table('tbproccesreport')
                ->select('Status', 'PDF_Location')
                ->where('ProccesName','=','Generate HotBill')
                ->where('colrelationID','=',$colrelationID)
                ->paginate(10);
                // dd($PDF[0]->PDF_Location);
                if($PDF){
                    if($PDF[0]->PDF_Location=='NULL'){
                        return response()
                        ->json(['Status' => 'Error',
                        ]);
                    }
                    if(($PDF[0]->Status)=='Success'){
                       return  response()
                            ->json(['Group PDF Localtion' => '/onip/bmbill_mount/bill_nfs/billgen/PDF_FIle/'.$colrelationID,
                            'PDF Localtion' => '/onip/invbill_mount/bill_nfs/fmt'.$PDF[0]->PDF_Location,
                            'ColRelationID' => $colrelationID
                            ]);
                    ...
                    return response()
                    ->json(['Status' => "Hot Bill is not completed. Please, Waiting(5min)...",
                    ]);
                ...
                return response()
                ->json(['Status' => "Please, Run Hot Bill",
                ]);
            }
            }catch(Exception $e){
...
public function Test($data){
    ...
    $conn = oci_connect('smart', 'Smart_%002', '10.12.5.191:1526/suseora');
        ...
        $cust_info=$this->QueryData(967369050); 
        $ACCT=$cust_info[0];
        $this->NGBSSService = new NGBSSService($ACCT, "en");
        $Task_ID=$this->NGBSSService->HotBill($ACCT);
    $x++;
